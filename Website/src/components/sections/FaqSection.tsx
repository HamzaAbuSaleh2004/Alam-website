...
export function FaqSection() {
  ...
  const filteredItems = t.faq.items.filter((item) => {
    const matchesCategory =
      activeCategory === "allCategory" || item.cat === activeCategory;
    const matchesSearch =
      searchQuery.trim() === "" ||
      ...
      <Container>
        <div className="mb-10 text-center max-w-2xl mx-auto flex flex-col items-center">
          ...
          <div className="mt-6 w-full max-w-md relative">
            <input
              ...
              className="w-full rounded-full border border-border bg-bg px-5 py-3 text-sm text-heading placeholder:text-fg-muted focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 transition-all"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-fg-muted hover:text-heading rtl:right-auto rtl:left-4"
              >
                ✕
              </button>
            )}
        ...
        <div className="mx-auto max-w-3xl flex flex-col gap-3">
          {filteredItems.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-border p-8 text-center text-fg-muted text-sm">
              {t.faq.emptyState}
            </div>
          ) : (
